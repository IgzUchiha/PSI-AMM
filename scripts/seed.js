...
async function main() {
  ...
  // fetch PSI Token 
  // Fetch PSI Token
  const psi = await ethers.getContractAt('Token', config[chainId].psi.address)
  console.log(`PSI Token fetched: ${psi.address}\n`)
  ...
  // Send psi tokens to investor 1
  transaction = await psi.connect(deployer).transfer(investor1.address, tokens(10))
  ...
  // Send psi tokens to investor 3
  transaction = await psi.connect(deployer).transfer(investor3.address, tokens(10))
  await transaction.wait()
  ...
  transaction = await psi.connect(deployer).approve(amm.address, amount)
  await transaction.wait()
  ...
  /////////////////////////////////////////////////////////////
  // Investor 1 Swaps: PSI --> USD
  //

  console.log(`Investor 1 Swaps...\n`)

  // Investor approves all tokens
  transaction = await psi.connect(investor1).approve(amm.address, tokens(10))
  await transaction.wait()
  ...
  /////////////////////////////////////////////////////////////
  // Investor 3 Swaps: PSI --> USD
  //

  console.log(`Investor 3 Swaps...\n`)

  // Investor approves all tokens
  transaction = await psi.connect(investor3).approve(amm.address, tokens(10))
  await transaction.wait()
